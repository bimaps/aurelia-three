...
var log = getLogger('three-measure-tool');
var logThree;
var ThreeMeasureTool = (function (_super) {
            ...
            for (var _a = 0, intersections_1 = intersections; _a < intersections_1.length; _a++) {
                ...
                if (object instanceof THREE.Mesh && intersection.face) {
                    ...
                    if (this.snapping === 'summit') {
                        ...
                        if (geometry instanceof THREE.Geometry) {
                            var lineA = new Line3(geometry.vertices[face.a], geometry.vertices[face.b]);
                            var lineB = new Line3(geometry.vertices[face.a], geometry.vertices[face.c]);
                            var lineC = new Line3(geometry.vertices[face.b], geometry.vertices[face.c]);
                            ...
                            var lineA = new Line3(new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, face.a), new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, face.b));
                            var lineB = new Line3(new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, face.a), new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, face.c));
                            var lineC = new Line3(new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, face.b), new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, face.c));
                            var vA = new THREE.Vector3();
                            var vB = new THREE.Vector3();
                            var vC = new THREE.Vector3();
                            lineA.closestPointToPoint(intersection.point, true, vA);
    ...
    ThreeMeasureTool.prototype.generateMeasureObject = function (measure) {
        ...
        if (measure.type === 'distance') {
            ...
            var sprite = new SpriteText2D(value.toString(), {
            ...
            var centroid = ThreeUtils.centroidFromObject(surfaceMesh);
            ...
            var centroid = ThreeUtils.centroidFromObject(volumeMesh);
            var spritePosition = centroid;
            sprite.position.set(spritePosition.x, spritePosition.y, spritePosition.z);
            sprite.userData._type = '__measure_current-measure-label__';
            sprite.material.depthTest = false;
    ...
    ThreeMeasureTool.prototype.addCoordinate = function (coord) {
        ...
        if (this.currentMeasure.type === 'distance' && this.currentMeasure.coords.length === 2) {
            ...
            if (this.currentMeasure.coords.length > 2) {
                ...
                var area = ShapeUtils.area(contour);
